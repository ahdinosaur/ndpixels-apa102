...
//
// TODO performance
// - initialize with properly sized Buffer, set by index
...
var _pixelsToApa102Buffer = cwise({
  args: ['scalar', 'shape', { blockIndices: -1 }],
  pre: function (bl) {
    // start frame
    bl.append(Buffer([0x00, 0x00, 0x00, 0x00]))
  },
  body: function (bl, shape, pixel) {
    ...
    var green = pixel[1]
    var blue = pixel[2]
    bl.append(Buffer([prefix, blue, green, red]))
  },
  post: function (bl, shape) {
  ...
  funcName: '_pixelsToApa102Buffer'
})

function pixelsToApa102Buffer (pixels) {
  return _pixelsToApa102Buffer(BufferList(), pixels)
}
